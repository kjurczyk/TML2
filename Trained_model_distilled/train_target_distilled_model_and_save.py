...
"""
files = [
      '10_labels_from_first_100.gz', '10_distilled_images_from_first_100.gz',
...
with gzip.open(files[1], 'rb') as imgpath:
    ...
    imgpath.read(), np.uint8).reshape(len(labels_from_first_100), 28, 28, 3)
...
"""
def train_model(model, x_train, y_train, x_test, y_test, num_epochs, batch_size=64, optimalg='sgd', lossfn='categorical_crossentropy', metrics_list=['accuracy'], verbose=False):
...
save_model(model,"30e_025rc_300h_distilled")
