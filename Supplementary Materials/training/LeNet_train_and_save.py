...
distilled = True
training_data_filename = '10_distilled_batch_size_64_400epochs' # change the filename
if distilled:
...
with gzip.open(dirPathDistilled + files[0], 'rb') as lbpath:   # training
    train['labels'] = np.frombuffer(lbpath.read(), np.uint8)
    print(train['labels'].shape)
    # print(labels_from_first_100)

with gzip.open(dirPathDistilled + files[1], 'rb') as imgpath:  # training
